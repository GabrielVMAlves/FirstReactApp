...
export default class Database {
            ...
            SQLite.echoTest().then(() => {
                ...
                SQLite.openDatabase(
                    ...
                    db.executeSql('SELECT 1 FROM Product LIMIT 1').then(() => {
                        ...
                        console.log("Database not yet ready ... popuating data");
                        db.transaction((tx) => {
                ...
                console.log("echoTest failed - plugin not funcional");
            })
        });
    };
    ...
    listProduct() {
        return new Promise((resolve) => {
            ...
            this.initDB().then((db) => {
                db.transaction((tx) => {
                    tx.executeSql('SELECT p.prodId, p.prodName, p.prodImage FROM Product p',[])
                    .then(([tx,results]) => {
    ...
    productById(id) {
        ...
        return new Promise((resolve) => {
            this.initDB().then((db) => {
                db.transaction((tx) => {
                    tx.executeSql('SELECT * FROM Products WHERE prodId = ?', [id])
                    .then(([tx,results]) => {
                        console.log(results);
    ...
    updateProduct(id, prod) {
        return new Promise((resolve) => {
            this.initDB().then((db) => {
                db.transaction((tx) => {
                    tx.executeSql('UPDATE Product SET prodName = ?, prodDesc = ?, prodImage = ?, prodPrice = ?, WHERE prodId = ?',
